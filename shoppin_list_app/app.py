...
def slist_edit_page(slist_id):
    ...
    if request.method == 'GET':
        form = {'title': slist.title,
                'year': slist.year if slist.year else ''}
        return render_template('add_shopping_list.html.html', form=form,
                               min_year=1887, max_year=datetime.now().year)
    else:
        slist.title = request.form['title']
        slist.year = int(request.form['year']) if request.form['year'] else None
        current_app.store.update_movie(slist)
        flash('Movie data updated.')
        return redirect(url_for('site.product_page', slist_id=movie._id))
